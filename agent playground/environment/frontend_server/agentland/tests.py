...
class AgentLandP0Tests(TestCase):
    ...
    def test_deterministic_dispatch_is_ordered_idempotent_and_restart_safe(self):
        ...
        self.assertEqual([item["sequence"] for item in events], list(range(1, 18)))
        self.assertEqual(events[10]["type"], "file.modified")
        ...
        self.assertEqual(len(duplicate.json()["events"]), 17)
    ...
    def test_city_projection_is_derived_from_persisted_events(self):
        ...
        self.assertContains(page, "the_ville_jan7.json")
        ...
        self.assertEqual(frames[9]["buildings"]["code_factory"], "active")
        self.assertEqual(frames[9]["evidence"]["files"], [])
        self.assertEqual(frames[10]["evidence"]["files"][0]["path"], "src/rooms.js")
